...
namespace HeapSort
{
    class MyFileList : DataList
    {
        ...
        {
            Byte[] data;
            double d1 = ReturnValueByIndex(a);
            double d2 = ReturnValueByIndex(b);
            fs.Seek(a * 12 + 4, SeekOrigin.Begin);
            data = BitConverter.GetBytes(d2);
            fs.Write(data, 0, 8);
            fs.Seek(b * 12 + 4, SeekOrigin.Begin);
            data = BitConverter.GetBytes(d1);
            fs.Write(data, 0, 8);
        }
